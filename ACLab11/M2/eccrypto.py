...
class EllipticCurvePoint(Point):
    ...
    def scalar_mult_timed(self, n: int) -> Tuple[Point, list[int]]:
        n_bin = "{0:b}".format(n)
        timings = list()
        P = self
        for bit in n_bin[1:]:
...
class ECDSA:
    ...
    def sign_timed(self, msg_bytes: bytes) -> Tuple[bytes, bytes, list[int]]:
        h = int.from_bytes(SHA256.new(msg_bytes).digest(), 'big') % self.ec.n
        k = secrets.randbelow(self.ec.n)
        kP, timings = self.ec.G.scalar_mult_timed(k)
        r = kP.x % self.ec.n
        s = pow(k, -1, self.ec.n) * (h + self.d * r) % self.ec.n

        return r.to_bytes(r.bit_length() // 8 + 1, 'big'), s.to_bytes(s.bit_length() // 8 + 1, 'big'), timings
    ...
    def verify(
        ...
        Z_prime = self.ec.G.scalar_mult_timed(u1)
