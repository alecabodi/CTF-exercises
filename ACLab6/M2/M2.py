...
class SHAzam:
    ...
    def setThings(self, length):
        self.hash = []
        for i in range(5):
            self.hash += struct.unpack(b'>I', bytes.fromhex(mac)[i * 4:(i + 1) * 4])
        print(self.hash)
        self.length = length
...
print(response)
cmd = response.get("authenticated_command")
mac = response.get("mac")

sha = SHAzam()
sha.setThings(64)

sha.update(b'&command=flag')
cmd_flag = sha.digest().hex()

m_len = len(b'command=hello&arg=world') + 16
padded_data = b'\x80'
padded_data += b'\x00' * ((56 - (m_len + 1) % 64) % 64)
m_bit_len = m_len * 8
padded_data += struct.pack(b'>Q', m_bit_len)
...
request = {
    ...
    "authenticated_command": (b"command=hello&arg=world" + padded_data + b"&command=flag").hex(),
    "mac": cmd_flag
}
